...
const CourseDetails = () => {
  ...
  const { isLoggedIn, setSelectedCourse } = useAppStore();
  ...
  const handleEnroll = () => {
    ...
    // Save selected course to store for checkout
    setSelectedCourse({
      id: String(course.id),
      name: course.title,
      category: `Hạng ${course.licenseType}`,
    ...
    if (isLoggedIn) {
      ...
      // Not logged in → go to auth, after login will redirect to checkout
      navigate('/auth');
    }
  };
  ...
  if (!course) {
    return (
      <>
        <UserNavbar />
        <div className="min-h-screen flex items-center justify-center">
          <p className="text-on-surface-variant text-lg">Không tìm thấy khóa học.</p>
        </div>
      </>
    );
  }
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
            <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-on-primary-container text-xs font-bold uppercase tracking-wider mb-6">
                ...
                Hạng {course.licenseType} · {course.duration}
              </div>
              ...
              <p className="text-3xl font-black text-secondary-container mb-8">{formatPrice(course.price)}</p>
              <div className="flex flex-wrap gap-4">
                <button
                  onClick={handleEnroll}
                  className="inline-flex justify-center items-center text-center bg-surface-container-lowest text-primary hover:bg-surface-bright px-8 py-4 rounded-xl font-bold text-lg shadow-xl shadow-black/10 active:scale-95 transition-all"
                >
                  Đăng ký khóa này
                </button>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            <div className="lg:w-[70%] space-y-16">
                    ...
                    <div className="flex items-start gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          <span className="px-4 py-2 bg-surface-container-highest rounded-full text-xs font-semibold text-primary">Xe đời mới</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="relative h-[400px] w-full rounded-[40px] overflow-hidden group">
                <img alt="Luxury car interior" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDlXu_8M3S8HMLy7qZLR-Gb_P2NkWIKxBqOV_EBPz5ctTR20BjARyKULrpZ34x4bfirEbs37_QmMPb4NOD25zGZad8Pbw2jKnQCWbJ8ka2SiV1IaVirf-zqy07FlihDW5H_GwsBgXW7rifC1EdFFuw5thYdWBNB1QK_W8sBVLaczLFzAEH5yeKEFdvM1gVTXJSwaCY4ftpiZJe-5XSgbQmFGmWvubS65OdznrVaabQIN3I540r2f_0A0R5eKJQ-pUABjca6hdzSL6w"/>
